...
/**
 * task_lists delete cleanup — prevents dangling user.household_slugs refs.
 *
 * users.household_slugs is a JSON OBJECT mapping `{ slug: task_list_id }`
 * (not a multi-relation, not an array). PB does NOT cascade-clean it when a
 * task_lists record is deleted, so the next list deletion would leave a
 ...
 *
 *   (a) JSON field "filter" — household_slugs is a JSON field, not a
 *       multi-relation, so the `?=` "any-equals" operator doesn't apply.
 *       We use `household_slugs ~ "<id>"` (substring on the JSON-encoded
...
onRecordAfterDeleteSuccess((e) => {
  ...
  try {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      const slugs = toPlainObject(user.get("household_slugs"));
      ...
      user.set("household_slugs", pruned);
      $app.save(user);
      touched++;
    }
  } catch (err) {
    // Don't let cleanup failures swallow the delete success. Log and move on.
    console.log("task-list-cleanup: error scrubbing refs for " + listId + ": " + (err && err.message ? err.message : err));
  }

  if (touched > 0) {
    console.log("task-list-cleanup: pruned " + listId + " from " + touched + " user(s)");
  }
