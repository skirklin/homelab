...
3. **OAuth surface gating.** `mcpHostGate` in `index.ts:55-61` 404s anything outside `MCP_ALLOWED_HOSTS` (prod = `mcp.tail56ca88.ts.net`). Mounted before `authMiddleware` so OAuth discovery is reachable but only on tailnet.
