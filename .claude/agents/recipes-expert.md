...
- **Data shape change.** Touch PB migrations under `infra/pocketbase/pb_migrations/`, the mapper (pocketbase/recipes.ts:41 `recipeFromRecord`), `recipeDataSchema` in mcp.ts:646, and the surgical ops at mcp.ts:540-700.
...
- **Optimistic writes.** PB uses `wrapPocketBase` from `packages/backend/src/wrapped-pb/` for optimistic writes + cache reads.

## Grounding before action

1. Read the adapter: pocketbase/recipes.ts (mutations via `this.wpb.collection`, optimistic via the wrapped-pb layer).
...
4. PB subscription flow (pocketbase/recipes.ts:308-369) buffers initial `recipe_boxes` + `recipes` events into one combined `onBox(box, recipes)`.

## Quality standards

- Schema changes touch all the layers (PB migration, the adapter, `recipeDataSchema`, surgical MCP ops) in one PR.
- Scraper bug fixes ship with a captured JSON-LD fixture and unit test — never smoke-test against a live URL only.
- AI enrichment writes to `recipes.pending_changes` for user review (not the live recipe); it does not validate against `recipeDataSchema`. Surface enrichment failures rather than half-saving.
- Visibility tests cover both directions (private recipe in public box, public recipe in private box).
...
- **Box visibility ≠ recipe visibility** — private setting wins; test both directions (private recipe in public box, vice versa).
...
- **`deleteBox` doesn't scrub stale IDs** from other users' subscriptions (pocketbase/recipes.ts) — reads filter naturally.