...
dataRoutes.get("/boxes", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Failed to fetch boxes" }, 500);
  }
});

// List recipes in a box
dataRoutes.get("/recipes", async (c) => {
  const pb = userClient(c.get("userToken"));
...
dataRoutes.get("/recipes/:id", async (c) => {
  const pb = userClient(c.get("userToken"));
...
dataRoutes.get("/shopping/lists", async (c) => {
  const pb = userClient(c.get("userToken"));
  const userId = c.get("userId") as string;
  try {
...
dataRoutes.get("/shopping/items", async (c) => {
  const pb = userClient(c.get("userToken"));
...
dataRoutes.post("/shopping/items", async (c) => {
  const pb = userClient(c.get("userToken"));
  const userId = c.get("userId") as string;
