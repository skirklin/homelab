...
describe("updateTrip", () => {
  it("updates trip fields", async () => {
    ...
    await travel.updateTrip(tripId, tripUpdatesToBackend({ destination: "Munich", status: "Researching", region: "Europe" }));
...
// ── Activity reflection (experiencedAt timestamp) ──
// verdict / personal_notes retired — those now live in travel_notes.

describe("activity reflection fields", () => {
  it("stores and reads back experiencedAt", async () => {
    const user = await createTestUser(ctx);
    const cleanup = new TestCleanup();
    cleanup.bind(ctx.pb);
    ...
    await travel.updateActivity(
      activityId,
      activityUpdatesToBackend({ experiencedAt: experienced }),
    );
