 ...
 * doing?", called by both the HabitBoard view and the MCP progress tool. It
 * interprets existing `life_events` against a goal definition; it never writes.
 *
 * All period math is in the browser-LOCAL timezone (the same convention the
 * dashboard's day nav and DayTimeline use). Weeks are Sunday-start, matching
 * Visualizations.tsx / SessionStreakGrid.
 *
...
import { startOfDay, endOfDay } from "./shapes";
