...
- **Mirror bootstrap pre-registration race — harden so a fresh subscriber can't miss a just-published event (PARKED branch: `worktree-agent-a519deaaf907ae6ba`, commit `c208d82`)** — `bootstrap()` emits initial state off `getFullList` and marks the slice `ready` *before* the SSE `subscribe()` POST to `/api/realtime` is registered server-side; PocketBase does NOT replay pre-registration events, so any event published in that window is lost until the next `resync()`. Low-urgency in prod (a real user doesn't race their own subscribe against another device's sub-100ms write, and resync self-heals), but real. The parked branch implements the right shape — eager-paint-from-cache stays first/ungated, SSE registration overlaps the fetch, `ready`+fresh-emit await registration (raced against a 10s timeout fallback). **It is NOT merge-ready**: an adversarial review found that attaching the listener before the fetch resolves opens a pre-ready window where bootstrap's consolidation steps corrupt the live-SSE state — `reconcileGhosts` tombstones a pre-ready SSE *create* (and persists the wrong tombstone to IDB), the seed loop resurrects a pre-ready SSE *delete*, and a sort+limit `slice.records` replace drops a pre-ready event. The fix direction is settled: track the ids the live listener saw during the bootstrap window and have the seed loop, `reconcileGhosts`, and the sort+limit replace treat those as authoritative (same "don't let a stale fetch clobber fresher live data" principle the existing seed guard uses, extended to tombstones + the limit window), with cold-cache pre-ready create/update/delete tests across all three slice shapes. Pick the branch up, apply that, re-review before merge.
