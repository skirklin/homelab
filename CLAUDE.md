...
- **VPS state outside the repo** — sysctl tweaks (e.g. `fs.inotify.max_user_watches`), 1Password SSH agent config, k3s install script invocation, etc. are currently undocumented manual steps. If the box ever gets rebuilt, recovering takes archaeology. Worth either a bootstrap script under `infra/scripts/` or a small Ansible playbook. (Partially addressed: `api-secrets` is now reproducible from `.env` via `infra/scripts/push-secrets.sh`.)
