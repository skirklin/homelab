...
- **First command to run inside your worktree: `./infra/scripts/worktree-init.sh`.** It symlinks `node_modules/` + `packages/*/dist/` from the parent repo so `pnpm exec tsc`, tests, and other workspace commands work without a fresh install. Re-running is a no-op; pass `--clean` before final review to drop the symlinks. **If your change ADDS a dependency** (new `package.json` entry), the symlinked `node_modules` resolves it inside the worktree but the *main checkout's* `node_modules` won't have it — so after merging, run `pnpm install` in the main checkout before `./infra/deploy.sh`, or the pre-deploy gate's `pnpm typecheck` (which runs in main) fails with `Cannot find module`.
...
- **Pre-deploy gate**: `./infra/deploy.sh` runs `pnpm typecheck` + `pnpm test` + `pnpm test:playwright` before any image build. The gate calls `infra/test-env.sh url --pb` and `--api` to discover the worktree-specific PB / API URLs, then invokes `infra/test-env.sh fresh` to cycle (down+up) the test env so each gate run starts against a freshly-rebuilt PB — auth/realtime/index state can't drift across gates. The image stays cached so the cycle is ~10-20s. `pnpm test:playwright` is `turbo run test:playwright --concurrency=1` — workspaces opt in by defining the script in their own `package.json` (currently only `apps/shopping/app`; turbo skips packages without it). Bypass with `--skip-tests` (or `SKIP_TESTS=1`) only for hotfixes — fires a loud red warning + 3s pause before continuing.
...
- **Mirror reconciliation — finish making convergence structural (steps 2–3 of 3).** The recurring mirror bug class is that *one* principle — "the newest server observation per record wins; a bulk fetch is older than any SSE event after it" — is re-implemented at every consolidation site with its own ad-hoc flag (`hasServerSnapshot`/`isHydrated` seed guard, `sseTouchedDuringBootstrap`, `consolidated`, the resync queue-scan). The invariant is global-and-emergent (correct only if every site agrees) instead of local-and-total. **Step 1 is DONE and shipped**: a property-based convergence fuzzer ([`packages/backend/src/wrapped-pb/mirror-convergence.fuzz.test.ts`](packages/backend/src/wrapped-pb/mirror-convergence.fuzz.test.ts), fast-check, black-box over `watch` + a stub PB that models PB's no-replay-before-registration; 1000 cases/sec, gate-safe) — it immediately found two real convergence bugs three review rounds had missed (resync queue-only ghost; sort+limit pre-ready demotion under-fill), both fixed. It is the guard rail for the rest. **Step 2**: funnel every server-truth ingestion (bootstrap rows, SSE events, write-acks, resync) through ONE monotonic `applyServer(record|tombstone, origin, seq)` that rejects anything older than what it holds — collapses all the scattered freshness flags into per-record provenance+version, and makes bootstrap "just more observations" (subscribe-then-feed-both), so the pre-registration window stops being a phase. **Step 3**: dissolve `slice.records` into a pure sort+limit view over the queue (it's only kept because the client doesn't hold the whole collection for windowed queries — treat it as a refetch-hinted cache, never SSE-patched). Do steps 2/3 against the fuzzer (extend it as needed); it's structurally blind to non-convergence hazards (refetch storms, leaks) so keep pairing it with a focused review. NOTE the latent footgun the resync queue-scan now relies on: a filter-only wildcard consumer's `predicate` must select the same set as its server `filter` string or resync can false-tombstone — see the `INVARIANT:` comment in `mirror.ts` resync and `apps/.../pocketbase/shopping.ts`.
