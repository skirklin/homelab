...
export interface EventExtraction {
  ...
  /** Narrative context note (what happens before/after) */
  sequenceNote?: string;
...
export interface CharacterEntity {
  ...
  /** Event IDs this character is involved in (in story order) */
  eventIds: string[];
...
export interface CharacterRelationship {
  ...
  /** Target character's name for display */
  targetName: string;
  relationship: string; // "spouse", "employer", "rival", etc.
  /** Event IDs where both characters are involved together */
...
export interface CharacterMention {
  ...
  /** Relationships to other characters revealed in this mention */
  relationshipsMentioned: Array<{
    target: string; // Target character name
    relationship: string; // e.g., "spouse", "employer", "rival"
  }>;
}
...
export interface PlotThreadTouch {
  threadId: string;
  /** Brief name for thread matching (e.g., "Emma Hartley murder investigation") */
  name: string;
  action: 'introduced' | 'advanced' | 'complicated' | 'resolved';
  description: string;
  location: TextLocation;
...
export interface TokenUsage {
  /** Tokens used in discovery pass */
  discovery: {
    inputTokens: number;
    outputTokens: number;
  };
  /** Tokens used in extraction pass */
  extraction: {
    inputTokens: number;
    outputTokens: number;
  };
