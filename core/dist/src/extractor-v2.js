 ...
 */
export async function extractFromChunksV2(chunks, options = {}) {
    ...
    for (let i = 0; i < chunks.length; i += concurrency) {
        ...
        const batchResults = await Promise.all(batch.map(async (chunk, idx) => {
            const result = await extractFromChunkV2(client, model, chunk, offsets[idx]);
            completed++;
 ...
 */
async function extractFromChunkV2(client, model, chunk, startOffset) {
 ...
 */
function processRawExtraction(raw, chunk) {
    const findLocation = (quote) => {
    ...
    // Process characters
    const characterMentions = (raw.characters ?? []).map((c) => ({
    ...
    // Process facts
    const facts = (raw.facts ?? []).map((f, idx) => ({
    ...
    const plotThreads = (raw.plotThreads ?? []).map((pt) => ({
    ...
    // Process setups
    const setups = (raw.setups ?? []).map((s, idx) => ({
 ...
 */
function findTextLocation(quote, chunkText, chunkId) {
 ...
 */
function fuzzyFindQuote(quote, text) {
    // Normalize both strings
    const normalizeForSearch = (s) => s.toLowerCase().replace(/[^\w\s]/g, '').replace(/\s+/g, ' ').trim();
 ...
 */
function getHumanReadableLocation(text, offset) {
...
//# sourceMappingURL=extractor-v2.js.map