 ...
 */
export function aggregateEntities(chunks) {
 ...
 */
function collectCharacterMentions(chunks) {
    const mentions = [];
 ...
 */
function deduplicateCharacters(mentions, chunks) {
    ...
    for (const mention of mentions) {
        ...
        if (existingKey) {
            ...
            // Add appearance
            const existingAppearance = char.appearances.find((a) => a.chunkId === mention.chunkId);
 ...
 */
function findMatchingCharacter(normalizedName, characterMap) {
    // Exact match
 ...
 */
function linkCharacterIds(chunks, nameToId) {
 ...
 */
function aggregateLocations(chunks) {
        ...
        for (const fact of chunk.extraction.facts) {
            if (fact.category === 'location') {
                ...
                if (locationMap.has(normalizedName)) {
                    ...
                    const existingAppearance = loc.appearances.find((a) => a.chunkId === chunk.id);
                    if (existingAppearance) {
 ...
 */
function aggregateObjects(chunks) {
 ...
 */
function extractObjectName(description) {
 ...
 */
function aggregatePlotThreads(chunks) {
    ...
    for (const chunk of chunks) {
        for (const touch of chunk.extraction.plotThreads) {
            ...
            for (const [, thread] of threadMap) {
                if (thread.name.toLowerCase().includes(normalizedName.slice(0, 20)) ||
