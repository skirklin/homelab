 ...
 * All keys are stored under "kirkl:groceries:{key}" to avoid collisions.
 */

const NAMESPACE = "kirkl:groceries";
...
export const appStorage = {
  ...
  migrateFromLegacy(): void {
    const oldKey = "groceries-last-list";
    const oldValue = localStorage.getItem(oldKey);
    if (oldValue) {
      this.set("lastList", oldValue);
      localStorage.removeItem(oldKey);
      console.log("[storage] Migrated groceries-last-list to namespaced storage");
    }
  },
};
