...
export function ActivityCard({ activity, activeEntry, userId, logId }: ActivityCardProps) {
  ...
  const handleToggle = async () => {
    ...
    try {
      if (activeEntry) {
        ...
        message.success(`${activity.label} stopped`);
      } else {
        await startActivity(activity.id, userId, logId);
        message.success(`${activity.label} started`);
      }
    } catch (error) {
  ...
  return (
    <Card $color={activity.color} $active={!!activeEntry}>
